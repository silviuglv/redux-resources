...
export const fieldApi = {
	getFields: (packageId: string, params: GetFieldsRequest = {}) => {
		return axios({
			url: `${config.dev_env === true ? config.apiMock : config.apiBase}/packages/${packageId}/fields`,
			method: 'GET',
			params,
		})
	},
	...
	updateField: (packageId: string, fieldId: string, data: UpdateFieldRequest) => {
		return axios({
			url: `${config.dev_env === true ? config.apiMock : config.apiBase}/packages/${packageId}/fields/${fieldId}`,
			method: 'PUT',
			data,
		})
	},
